...
class GASOLINE {
	...
	private static void solve() {
		int n=scan.nextInt();
        long gasTank[]=readLongArray(n);
        ArrayList<Pair> al=new ArrayList<>();
        for(int i=0;i<n;i++){
            long temp=scan.nextLong();
            al.add(new Pair(temp,i));
        }
        Collections.sort(al);
        int count=n;
        long ans=0;
        for(Pair p:al){
            int temp=p.pos;
            long costLitre=p.cost;
            long tempGas=gasTank[temp];
            if(tempGas>0 && tempGas<n){
                ans=ans+ tempGas*costLitre;
                n=(int) (n-tempGas);
            }
            else if(tempGas>0 && tempGas>=n){
                ans=ans+ n*costLitre;
                break;
            }
        }
